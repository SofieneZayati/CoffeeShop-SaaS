...
The project contains four focused demo accounts. Select any actor on the login screen to fill their credentials automatically.
...
| Customer — Mariem Ben Ali | `mariem@greencoffee.tn` | `GreenGuest26!` | Customer storefront | Menu/cart, table-session checkout, order tracking, bookings, rewards, events, favorites and profile; no staff shell is rendered |

The login screen also has a no-account table-ordering entry. It opens `#table` in guest mode; scanning/selecting a table creates the simulated short-lived device session.
...
- Order transitions are station-specific: owners/managers can enter waiter orders and serve; baristas, managers, and owners can accept tickets into preparation.
- Table checkout requires an active, unexpired scan session and stores the table/session identity on the order.
- Owners/managers can enter a manual order and clear a table to revoke its previous device session; customer-submitted tickets wait for staff acceptance.
